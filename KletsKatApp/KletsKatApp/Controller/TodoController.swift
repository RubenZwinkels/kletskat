...
//            self.todoItems.append(item)
    ...
    public func deleteTodoItem(_ id: UUID, completion: @escaping (Bool) -> Void) {
        guard let url = URL(string: apiUrl) else {
            completion(false)
            return
        }
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            if let error = error {
                print("Fout bij het deleten van todo-item: \(error)")
                completion(false)
                return
            }
        ...
        task.resume()
        removeTodoItem(by: id)
    }
    
    private func removeTodoItem(by id: UUID) {
